...
export default function AboutPageContent() {
  ...
  // Track scroll direction for lanyard entrance animation
  const prevScrollYRef = useRef(0);
  const currScrollYRef = useRef(typeof window !== 'undefined' ? window.scrollY : 0);

  useEffect(() => {
    const onScroll = () => {
      prevScrollYRef.current = currScrollYRef.current;
      currScrollYRef.current = window.scrollY;
    };
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const enteringFromTop = currScrollYRef.current > prevScrollYRef.current;
            const startY = enteringFromTop ? -140 : 140;
            controls.set({ y: startY, opacity: 0 });
            controls.start({ y: 0, opacity: 1, transition: { type: 'spring', stiffness: 120, damping: 20, mass: 0.7 } });
          }
        });
      },
      { threshold: 0.18 }
      ...
      <motion.section className="w-full max-w-6xl mx-auto" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <aside className="order-1 lg:order-1 flex justify-center lg:justify-start" aria-hidden>
            <motion.div ref={lanyardRef} initial={{ opacity: 0 }} animate={controls} className="w-full max-w-xs sm:max-w-sm lg:max-w-[360px]">
          ...
          <div className="order-2 lg:order-2 lg:pl-4">
            <div className="p-6 rounded-xl shadow-sm bg-transparent">
              <div className="flex flex-col gap-2">
                ...
                <p className="mt-4 text-sm leading-relaxed text-white/85">I build data-driven applications and polished interactive frontends. Currently focused on machine learning pipelines, model deployment, and creating UX that communicates complex data simply and beautifully.</p>
              </div>
                  ...
                  {Object.entries(SKILLS).map(([group, items]) => (
                    <div key={group}>
                      ...
                      <div className="flex flex-wrap">
                        {items.map((s) => (
                          <span key={s} className="mr-2 mb-2 inline-flex items-center text-xs px-3 py-1 rounded-full bg-white/6 text-white/90">{s}</span>
                        ))}
                      </div>
                    </div>
                  ))}
            ...
            <div className="p-6 rounded-xl shadow-sm bg-transparent">
              ...
              <div className="pt-4 mt-4 border-t border-white/6">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button onClick={() => router.push('#projects')} className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md text-sm bg-transparent border border-white/6">View Projects</button>
                </div>
              </div>
            </div>
          </aside>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div ref={containerRef} onMouseMove={handleMouseMove} className="relative p-6 rounded-xl shadow-sm bg-white/3">
            <h2 className="text-lg font-semibold mb-3">Detail Data Diri</h2>
            <pre className="font-mono text-sm leading-relaxed max-h-64 overflow-auto text-white/95"><code>{displayText}</code></pre>
          </div>
        </div>
