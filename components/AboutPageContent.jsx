...
const AboutPageContent = () => {
  ...
  const highlightCode = (text) => {
    ...
    return escaped
      ...
      .replace(/(\/\/.*?$)/gm, "<span class='text-[var(--color-text-secondary)]'>$1</span>");
  };

  return (
    <main className="min-h-screen w-full px-6 py-16 about-page">
      <motion.section className="w-full max-w-6xl mx-auto" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="order-2 lg:order-2 lg:pl-4">
            <div className="p-6 rounded-xl shadow-sm bg-[color:var(--color-bg-secondary)] border border-[color:var(--color-border-primary)]">
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {Object.entries(SKILLS).map(([group, items]) => (
                    <div key={group}>
                      <div className="text-xs font-semibold text-[var(--color-text-secondary)] mb-2">{group}</div>
                      <div className="flex flex-wrap">
                        {items.map((s) => (
                          <span key={s} className="mr-2 mb-2 inline-flex items-center text-xs px-3 py-1 rounded-full bg-[color:var(--color-bg-primary)] text-[var(--color-text-primary)]">
                            {s}
                          </span>
                        ))}
                      </div>
          ...
          <aside className="order-3 lg:order-3 lg:pl-6">
            <div className="p-6 rounded-xl shadow-sm bg-[color:var(--color-bg-secondary)] border border-[color:var(--color-border-primary)]">
              ...
              <div className="space-y-3">
                {SOCIAL.map((s) => (
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between gap-3 p-3 rounded-md hover:bg-[color:var(--color-bg-primary)] transition">
                    <div className="flex items-center gap-3">
                      <span className="text-teal-200">{s.icon}</span>
                      <div>
                        <div className="text-sm font-medium">{s.label}</div>
                        <div className="text-xs text-[var(--color-text-secondary)]">{s.href.replace(/https?:\/\//, '')}</div>
                      </div>
                    </div>
                    <div className="text-xs text-[var(--color-text-secondary)]">Open</div>
                  </a>
                ))}
              </div>

              <div className="pt-4 mt-4 border-t border-[color:var(--color-border-primary)]">
                <h4 className="text-xs font-semibold mb-2">Aksi Cepat</h4>
                <div className="flex flex-col gap-2">
                  <a href="https://drive.google.com/file/d/1iDmuU2lcHynCsv5ZvA_wh5NiQXcie-e-/view?usp=drive_link" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md" style={{ backgroundColor: 'var(--color-accent-light)', color: 'var(--color-bg-primary)', fontWeight: 600 }}>
                    <Download size={16} />
                    Download Resume / CV
                  </a>
                  <button onClick={() => router.push('#projects')} className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md text-sm bg-transparent border border-[color:var(--color-border-primary)]">
                    View Projects
                  </button>
                </div>
              </div>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          <div className="p-6 rounded-xl shadow-sm bg-[color:var(--color-bg-secondary)] border border-[color:var(--color-border-primary)]">
            ...
            <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">I build data-driven applications and polished interactive frontends. Currently focused on machine learning pipelines, model deployment, and creating UX that communicates complex data simply and beautifully.</p>

            <div className="mt-6">              <h3 className="text-sm font-semibold mb-2">Ringkasan</h3>              <ul className="text-sm text-[var(--color-text-secondary)] space-y-1">                <li>Mahasiswa Informatika — Universitas Amikom Yogyakarta</li>                <li>Fokus: Machine Learning, Big Data, Frontend Interactivity</li>                <li>Pengalaman: Bangkit Academy (ML Cohort 2024), HMIF</li>              </ul>            </div>          </div>          <div ref={containerRef} onMouseMove={handleMouseMove} className="relative p-6 rounded-xl shadow-sm bg-[color:var(--color-bg-secondary)] border border-[color:var(--color-border-primary)]">            <h2 className="text-lg font-semibold mb-3">Detail Data Diri</h2>            <pre className="font-mono text-sm leading-relaxed whitespace-pre-wrap text-[var(--color-text-primary)]"><code>{displayText}</code></pre>          </div>        </div>        {/* Experience full width */}
        <div className="mt-10">          <div className="p-6 rounded-xl shadow-sm bg-transparent">            <h2 className="text-lg font-bold mb-3">Experience</h2>            <ul className="space-y-4 text-sm">              <li className="flex justify-between border-b pb-2 border-white/6">                <div>                  <strong>Bangkit Academy</strong>                  <div className="text-[var(--color-text-secondary)] text-xs">Machine Learning Cohort 2024</div>                </div>                <div className="text-[var(--color-text-secondary)] text-xs">September 2024 – Desember 2024</div>              </li>              <li className="flex justify-between border-b pb-2 border-white/6">                <div>                  <strong>Himpunan Mahasiswa Informatika (HMIF)</strong>                  <div className="text-[var(--color-text-secondary)] text-xs">Hubungan Masyarakat</div>                </div>                <div className="text-[var(--color-text-secondary)] text-xs">Februari 2023 – September 2024</div>              </li>            </ul>          </div>        </div>
      </motion.section>
    </main>
  );
};
